...
new_data <-new_data %>% filter( !`Primary Site` %in% c(181, 188, 189,260))
table (new_data$"Primary Site")
...
new_data <- new_data %>% mutate(age_numeric = as.numeric(str_extract(`Age recode with single ages and 90+`,"[:digit:]+")))
...
table(new_data$"Grade Pathological (2018+)")

# Make nice grade variable
new_data2 <- new_data2 %>% mutate(final_grade = `Grade Pathological (2018+)`)
new_data2 <- new_data2 %>% mutate(final_grade = ifelse(`Grade Recode (thru 2017)` %in% "Poorly differentiated; Grade III", "3", final_grade))
new_data2 <- new_data2 %>% mutate(final_grade = ifelse(`Grade Recode (thru 2017)` %in% "Undifferentiated; anaplastic; Grade IV", "4", final_grade))
new_data2 <- new_data2 %>% mutate(final_grade = ifelse(`Grade Recode (thru 2017)` %in% "Moderately differentiated; Grade II", "2", final_grade))
new_data2 <- new_data2 %>% mutate(final_grade = ifelse(`Grade Recode (thru 2017)` %in% "Well differentiated; Grade I", "1", final_grade))
...
#042623: awaiting DH response to YOD categories 
...
new_data2<- new_data2%>%rename(race_eth=`Race and origin recode (NHW, NHB, NHAIAN, NHAPI, Hispanic) - no total`)
...
#Characteristics of Patient Population We are Working With (new_data5)
summary(new_data5$age_group_final)
new_data5summary <- new_data5 
summary(new_data5summary$age_numeric)
new_data5summary %>% 
...
```{r}
# CAN SEE (Had to change survival months to numeric
